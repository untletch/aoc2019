...
def line(wire):
    ...
    for i in wire.split(","):
        p1 = points(p1, i)
        states.append(list(p1))
    lines = []
    for i in range(len(states) - 1):
        lines.append(states[i] + states[i + 1])
    return lines
...
def is_intersect(l1, l2):
    ...
    x1, y1, x2, y2 = l1
    x3, y3, x4, y4 = l2
    t = LA.det(np.array([[x1 - x3, x3 - x4], [y1 - y3, y3 - y4]])) / LA.det(
        np.array([[x1 - x2, x3 - x4], [y1 - y2, y3 - y4]])
    )
    px, py = x1 + t * (x2 - x1), y1 + t * (y2 - y1)
    # remove points that intersect at origin
    if px == 0.0 and py == 0.0:
        return False
    # check if returns a whole number
    if str(px).endswith(".0") and str(py).endswith(".0"):
        return ceil(px), ceil(py)
    # check if float close to whole number
    if (
        ".999999" in str(px)
        and ".999999" in str(py)
        or ".999999" in str(px)
        and str(py).endswith(".0")
        or ".999999" in str(py)
        and str(px).endswith(".0")
    ):
        return ceil(px), ceil(py)
    return False


def manhattan_distance(p1):
    x1, x2 = 0, 0
    y1, y2 = p1
...
def ans():
    ...
    line2 = line(wire2)
    return min(
        manhattan_distance(is_intersect(i, j))
        for i in line1
        for j in line2
        if is_intersect(i, j)
    )
